...
export const GOD_TABLES: GodTableDef[] = [
  ...
  crud({
    key: 'marketplace_upload_batches',
    table: 'marketplace_upload_batches',
    label: 'Catalog upload batches',
    group: 'marketplace',
    description:
      'Supplier / reseller CSV·XLSX uploads. Staging only. PATCH /api/god/marketplace-uploads/:id to mark rows listed.',
    listColumns: [
      'id',
      'organization_id',
      'original_filename',
    ...
    searchColumns: ['original_filename', 'status', 'catalog_kind'],
    relatedKeys: ['marketplace_upload_rows', 'marketplace_listings', 'organizations'],
    canCreate: false,
    writeNote:
      'Create listings under the batch organization_id only. Prefer the agent PATCH API so listed_count rolls up. Status: pending → processing → listed | error | partial.',
  }),
  crud({
    key: 'marketplace_upload_rows',
    table: 'marketplace_upload_rows',
    label: 'Catalog upload rows',
    group: 'marketplace',
    description: 'One staged line item per spreadsheet row. Set status listed/error and marketplace_listing_id.',
    listColumns: [
      'id',
      'batch_id',
      'organization_id',
    ...
    searchColumns: ['sku', 'title', 'brand', 'status'],
    relatedKeys: ['marketplace_upload_batches', 'marketplace_listings'],
    canCreate: false,
    writeNote:
      'listed (alias imported) + marketplace_listing_id when the listing exists. error (alias failed) + error_message otherwise. Never invent a listing on another org.',
  }),
