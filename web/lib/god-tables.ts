...
export const GOD_TABLES: GodTableDef[] = [
  ...
  table({
    ...
    label: 'God email log',
    group: 'platform',
    description: 'God blast / shop-invite send log. Unsubscribe tokens are hidden.',
    pk: 'id',
    listColumns: ['id', 'created_at', 'organization_name', 'recipient_email', 'subject', 'template_key', 'unsubscribed_at'],
    searchColumns: ['recipient_email', 'organization_name', 'subject'],
    canCreate: false,
    canUpdate: false,
    canDelete: false,
    deleteConfirm: 'DELETE',
    readOnlyNote: 'Write this from Email blast / Invites only. Tokens stay hidden.',
  }),
  table({
    key: 'api_usage',
    table: 'api_usage',
