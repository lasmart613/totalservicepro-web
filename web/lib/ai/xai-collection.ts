...
/** True when the shared collection already has this filename (skip re-upload). */
...
export async function uploadPdfToTspCollection(opts: {
  ...
  try {
    if (await collectionHasDocumentName({ managementKey: manageKey, filename: name, collectionId })) {
