 ...
 * Parser reused from the held PR #84 draft. Live listings are created by
 * catalog-import.ts after a seller confirms the dry-run preview.
 */
...
export function validateCatalogFile(
  meta: CatalogFileMeta
): { ok: true; ext: 'csv' | 'xlsx' } | { ok: false; error: string } {
...
export type CatalogRowStatus = 'pending' | 'processing' | 'listed' | 'error';
...
export function parseCatalogSpreadsheet(
  ...
  if (!headers.some(Boolean)) {
    return { ok: false, error: 'The first row must be column headers (SKU, title, brand, …).' };
  }
...
export function catalogTemplateXlsx(kind: CatalogKind = 'part'): Buffer {
  const csv = buildCatalogTemplateCsv(kind);
  const grid = parseCsv(csv);
  return buildMinimalXlsx(grid);
}
